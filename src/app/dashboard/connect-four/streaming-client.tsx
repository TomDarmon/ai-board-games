...
import { ConnectFourBoard } from "~/components/connect-four/ConnectFourBoard";
...
export function ConnectFourStreamingClient() {
	...
	// Calculate which batch we need (each batch has 120 matches, showing 5 per page = 24 pages per batch)
	const batchNumber = Math.floor(currentPage / (BATCH_SIZE / MATCHES_PER_PAGE));
	const offset = batchNumber * BATCH_SIZE;
	const { data: matchesBatch, isLoading } = api.connectFour.getMatches.useQuery(
		{
			offset,
			limit: BATCH_SIZE,
		},
	...
	// Get the 5 matches for the current page from the batch
	const startIndex =
	...
	const formatConnectFourMove = (move: Record<string, unknown>) => {
		if ("column" in move) {
			return `Col ${move.column}`;
		}
	...
	if (viewMode === "live") {
		return (
			<GameMatchLayout
				...
				board={<ConnectFourBoard turns={turns} />}
				moveList={
					<GameMoveList turns={turns} formatMove={formatConnectFourMove} />
				}
				infoPanel={
					<MatchInfoPanel
			...
			<div className="space-y-6">
				...
				<GameReplay
					...
					renderBoard={(turns, currentStep) => (
						<ConnectFourBoard turns={turns} currentStep={currentStep} />
					)}
					formatMove={formatConnectFourMove}
				/>
			</div>
		);
	}
