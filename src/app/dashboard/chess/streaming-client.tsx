...
export function ChessStreamingClient() {
	...
	const { data: matchesBatch, isLoading } = api.chess.getMatches.useQuery({
		offset,
		limit: BATCH_SIZE,
	});
	...
	const formatChessMove = (move: Record<string, unknown>) => {
	...
	return (
		<div className="space-y-6">
			<GameLobbyStartCard
				playerXLabel="White (Player X)"
				playerOLabel="Black (Player O)"
				agentX={agentX}
				agentO={agentO}
			...
			<GameLobbyMatchHistory
				...
				formatMatchStatus={(match) => {
					if (match.status === GameStatus.finished) {
						...
						return match.winner === GameResult.X ? "White Won" : "Black Won";
					}
					return "In Progress";
				}}
			/>
